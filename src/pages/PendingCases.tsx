...
export default function PendingCases() {
  ...
  // Filter cases to only show Pending status
  const pendingCases = cases.filter(case_ => case_.status === 'Pending');

  const handleCaseSelect = (caseId: string) => {
    const selectedCase = pendingCases.find(c => c.id === caseId);
    if (selectedCase) {
  ...
  const handleCaseEdit = (caseId: string) => {
    const editingCase = pendingCases.find(c => c.id === caseId);
    if (editingCase) {
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Cases awaiting court proceedings or further action
          </p>
        </div>
        <ResponsiveCaseTable 
          cases={pendingCases}
